...
BEGIN
    ...
    -- 添加 TEMPLATE_ROLE 字段
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'APPATTACH_CATALOGUE_TEMPLATES' AND column_name = 'TEMPLATE_ROLE') THEN
        ALTER TABLE "APPATTACH_CATALOGUE_TEMPLATES" 
        ADD COLUMN "TEMPLATE_ROLE" integer NOT NULL DEFAULT 3;
        RAISE NOTICE '已添加 TEMPLATE_ROLE 字段';
    END IF;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM information_schema.check_constraints WHERE constraint_name = 'CK_ATTACH_CATALOGUE_TEMPLATES_TEMPLATE_PURPOSE') THEN
        ...
        CHECK ("TEMPLATE_PURPOSE" IN (1, 2, 3, 4, 5, 99));
        RAISE NOTICE '已添加模板用途约束';
    END IF;

    -- 添加模板角色约束
    IF NOT EXISTS (SELECT FROM information_schema.check_constraints WHERE constraint_name = 'CK_ATTACH_CATALOGUE_TEMPLATES_TEMPLATE_ROLE') THEN
        ALTER TABLE "APPATTACH_CATALOGUE_TEMPLATES" 
        ADD CONSTRAINT "CK_ATTACH_CATALOGUE_TEMPLATES_TEMPLATE_ROLE" 
        CHECK ("TEMPLATE_ROLE" IN (1, 2, 3));
...
BEGIN
    ...
    -- 模板角色索引
    IF NOT EXISTS (SELECT FROM pg_indexes WHERE indexname = 'IDX_ATTACH_CATALOGUE_TEMPLATES_TEMPLATE_ROLE') THEN
        CREATE INDEX "IDX_ATTACH_CATALOGUE_TEMPLATES_TEMPLATE_ROLE" 
        ON "APPATTACH_CATALOGUE_TEMPLATES" ("TEMPLATE_ROLE") 
        WHERE "IS_DELETED" = false;
...
BEGIN
    ...
    COMMENT ON COLUMN "APPATTACH_CATALOGUE_TEMPLATES"."TEMPLATE_ROLE" IS '模板角色（1=Root, 2=Navigation, 3=Normal）';
