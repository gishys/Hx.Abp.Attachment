...
ALTER TABLE "ATTACH_CATALOGUES" 
ADD COLUMN "TEMPLATE_VERSION" INTEGER NULL;

-- 2. 添加字段注释
COMMENT ON COLUMN "ATTACH_CATALOGUES"."TEMPLATE_VERSION" IS '关联的模板版本号，与TEMPLATE_ID一起构成完整的模板标识';

-- 3. 创建模板ID索引（如果不存在）
CREATE INDEX IF NOT EXISTS "IDX_ATTACH_CATALOGUES_TEMPLATE_ID" 
ON "ATTACH_CATALOGUES" ("TEMPLATE_ID");

-- 4. 创建模板ID和版本的复合索引
CREATE INDEX IF NOT EXISTS "IDX_ATTACH_CATALOGUES_TEMPLATE_ID_VERSION" 
ON "ATTACH_CATALOGUES" ("TEMPLATE_ID", "TEMPLATE_VERSION");
...
ON "ATTACH_CATALOGUES" USING GIN (
    to_tsvector('chinese', 
